...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        return http.csrf(AbstractHttpConfigurer::disable)
                ...
                .formLogin(formLogin -> {
                    formLogin.loginPage("/auth/login");             // GET
                    formLogin.loginProcessingUrl("/auth/login");    // POST
                    formLogin.defaultSuccessUrl("/index");
                    formLogin.failureUrl("/loginError");    // 로그인 오류 확인용 임시 페이지 (삭제 시 HomeController에서 같이 삭제)

                    // FIXME: 예외 출력용 임시 코드 (추후 변경 또는 삭제)
                    formLogin.failureHandler((request, response, exception) -> exception.printStackTrace());
